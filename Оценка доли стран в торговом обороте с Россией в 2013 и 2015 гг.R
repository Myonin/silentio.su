# Оценка доли стран в торговом обороте с Россией, сравнение 2013 и 2015 гг.,
# лидеры роста и аутсайдеры
# © silentio - gonzo data science lab | silentio.su - 21 сентября 2016 г.
...
# Вычисление доли в обороте
data_custom_percents <- subset(data_custom, Год == "2013" | Год == "2015",
                               select = c("Год", "Страна", "Оборот"))
for (i in 1:nrow(data_custom_percents)){
    ifelse(data_custom_percents$Год[i] == "2013", 
           data_custom_percents$Percents[i] <- data_custom_percents$Оборот[i]/
               sum(data_custom_percents$Оборот[data_custom_percents$Год == "2013"]),
           data_custom_percents$Percents[i] <- data_custom_percents$Оборот[i]/
               sum(data_custom_percents$Оборот[data_custom_percents$Год == "2015"]))  
}
...
# График "ТОП 10 стран по торговому обороту"
...
ggplot(data_custom_percents_top10, aes(x = as.factor(Страна), y = Percents, 
    ...
    coord_flip()+
    ggtitle("Внешнеторговый оборот России. Топ-10 стран")+
...
# Вычисление разницы доли в торговом обороте 2013 г. и 2015 г. по странам
data_custom_percents$Страна <- as.factor(data_custom_percents$Страна)
data_custom_percents$Оборот <- NULL
...
colnames(data_custom_percents_top) <- c("Страна",
                                        "Доля в торговом обороте, 2013 г., %",
                                        "Доля в торговом обороте, 2015 г., %",
                                        "Разница между 2015 и 2013 гг.")

# Таблица "Топ-10 стран, которые нарастили долю в торговом обороте с Россией"
grid.text("Топ-10 стран, которые нарастили долю в торговом обороте с Россией",
...
# Таблица "Топ-10 стран, которые потеряли долю в торговом обороте с Россией"
